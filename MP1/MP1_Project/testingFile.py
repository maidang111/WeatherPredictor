...
def gaussian_calculations(df, feat_name, feat_val, Y, label, X_test):
    df = df[df[Y] == label]
...
# y = weather EX sunny
def catagory_calculations(df, x_value, X, Y, y_val, X_test):
    y = df[Y]
    x = df[X]
...
def fetching_lastday(X, k, time):
    lastday_vals = X.iloc[-1 -k].to_dict()
    if 'time' in lastday_vals:
        lastday_vals['time'] = time + 600
...
def native_bayes(df, X_test, Y, categorical_features, numerical_features):
    ...
    x_prob = y_calculations(X_test, Y)
    time = X_test.iloc[-1]['time']
    
    # print(y_prob)

    for k in x_prob.keys():
        if k not in y_prob.keys():
            y_prob[k] = x_prob[k] + 0.1
    ...
    for k in range(1):
        term =  k * 4
        lastday = fetching_lastday(X_test, term, time)
        y = 0
        highest_prob = 0 
        prediction = ''
        for w in weathers:             
            ...
            for category in categorical_features:
                y *= catagory_calculations(df, lastday[category], category, Y, w, X_test)
            
            for feature in numerical_features:
                y *= gaussian_calculations(df, feature, lastday[feature], Y,  w, X_test)
            
            if y > highest_prob:
                prediction = w
                highest_prob = y
        # return prediction
        if highest_prob > 0.10:
            return prediction
        
        predictions.append(prediction)
    return k_NN(predictions)
...
def main():
    ...
    for file in sorted_files:
        ...
        value = native_bayes(df, X_test, Y, categorical_features, numerical_features)
